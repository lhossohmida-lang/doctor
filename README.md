...
- واجهة زبون عامة تسجل رقم الهاتف وتمنح الزبون رقم دور وQR، أو تعرض بطاقته إذا كان مسجلًا اليوم.
...
`queueCounters` هو عداد عام محدود لا يحتوي بيانات طبية أو مالية، ويستخدم لمنع تكرار أرقام الدور بين تسجيل الإدارة وتسجيل الزبون. `publicTickets` لا يحتوي المرض أو المبلغ أو حالة الدفع. `publicLookups` يسمح بالبحث برقم الهاتف دون السماح باستعراض قائمة الزبائن.
...
- `customer.html`: واجهة عامة للزبون. يدخل رقم الهاتف فقط؛ إذا كان مسجلًا اليوم تظهر بطاقته، وإذا لم يكن مسجلًا يتم إنشاء رقم دور جديد مع QR وبطاقة قابلة للطباعة. كما يمكن فتح رابط QR لعرض التتبع.
...
- عند تسجيل الزبون من الواجهة العامة يتم حفظ الاسم الافتراضي `زبون العيادة`، والمرض فارغ، والمبلغ `0`، وحالة الدفع `غير مدفوع` حتى يعدلها المدير لاحقًا من لوحة الإدارة.
